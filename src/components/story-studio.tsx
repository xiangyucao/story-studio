...
export function StoryStudio() {
  ...
  const [jsonExampleKind, setJsonExampleKind] = useState<"characters" | "world" | null>(null);
  ...
  return (
    <main className="app-shell">
      ...
      {worldImportPreview && <div className="modal-backdrop" onMouseDown={() => setWorldImportPreview(null)}><section className="settings-modal wide-modal" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">WORLD JSON IMPORT</span><h2>导入世界观与背景</h2></div><button className="icon-button" aria-label="关闭世界观 JSON 导入预览" onClick={() => setWorldImportPreview(null)}><X size={19} /></button></div><p className="modal-help">文件：{worldImportPreview.fileName}。只新增或更新，绝不会删除原有设定。同名设定更新非空字段；JSON 未提供的字段保留原值。</p><div className="import-summary"><span><strong>{worldImportPreview.data.worldEntries.length}</strong> 条设定</span><span><strong>{worldImportPreview.data.worldEntries.filter((entry) => entry.isCanon === true).length}</strong> 条明确标为硬设定</span></div><div className="import-preview-list">{worldImportPreview.data.worldEntries.map((entry) => { const existing = workspace.worldEntries.some((item) => normalizeWorldEntryName(item.name) === normalizeWorldEntryName(entry.name)); return <div className="import-preview-row" key={entry.name}><span className={existing ? "import-status update" : "import-status"}>{existing ? "更新" : "新增"}</span><div><strong>{entry.name}</strong><small>{entry.category || "保留原分类 / 默认背景"} · {entry.isCanon == null ? "保留 AI 使用方式" : entry.isCanon ? "硬设定" : "草稿"}</small></div></div>; })}</div><div className="modal-button-row"><button className="secondary-button" onClick={() => setWorldImportPreview(null)}>取消</button><button className="primary-button" disabled={busy} onClick={() => void importWorldFromJson()}>{busy ? <LoaderCircle className="spin" size={17} /> : <Upload size={17} />}确认导入</button></div></section></div>}
      {jsonExampleKind && <div className="modal-backdrop" onMouseDown={() => setJsonExampleKind(null)}><section className="settings-modal wide-modal" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">JSON EXAMPLE</span><h2>{jsonExampleKind === "characters" ? "人物与关系 JSON 示例" : "世界观与背景 JSON 示例"}</h2></div><button className="icon-button" aria-label="关闭 JSON 示例" onClick={() => setJsonExampleKind(null)}><X size={19} /></button></div><p className="modal-help">保存为 UTF-8 编码的 .json 文件后即可导入。导入只新增或更新同名内容，不会删除当前作品中的任何原有资料。</p><pre className="json-example-code">{JSON.stringify(jsonExampleKind === "characters" ? characterImportExample : worldImportExample, null, 2)}</pre><div className="modal-button-row"><button className="secondary-button" onClick={() => setJsonExampleKind(null)}>关闭</button><button className="primary-button" onClick={async () => { try { await navigator.clipboard.writeText(JSON.stringify(jsonExampleKind === "characters" ? characterImportExample : worldImportExample, null, 2)); setMessage("JSON 示例已复制到剪贴板"); } catch { setMessage("无法自动复制，请在示例代码中手动选择复制"); } }}>复制示例</button></div></section></div>}
    </main>
  );
}
