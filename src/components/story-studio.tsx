      ...
      {exportOpen && <div className="modal-backdrop" onMouseDown={() => setExportOpen(false)}>
        <section className="settings-modal export-modal" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div className="export-volume-picker">
            ...
            <div className="export-volume-list">{exportVolumeGroups.map((group) => { const key = exportGroupKey(group); return <label key={key}><input type="checkbox" checked={selectedExportVolumes.has(key)} onChange={() => setSelectedExportVolumes((current) => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); return next; })} /><span><strong>{group.volume?.title || "未归档章节"}</strong><small>{group.chapters.length} 章 · {group.chapters.reduce((sum, chapter) => sum + chapter.wordCount, 0).toLocaleString()} 字</small></span></label>; })}</div>
