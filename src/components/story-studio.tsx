...
function ImportDiffList({ items }: { items: ImportDiffItem[] }) {
  ...
  return <><div className="import-diff-summary"><span className="new">新增 {counts.new}</span><span className="changed">修改 {counts.changed}</span><span className="unchanged">未变化 {counts.unchanged}</span><span className="deleted">删除 {counts.deleted}</span></div><div className="import-preview-list import-diff-preview">{items.map((item, index) => <div className="import-preview-row" key={`${item.status}-${item.label}-${index}`}><span className={`import-status diff-${item.status}`}>{labels[item.status]}</span><div><strong>{item.label}</strong><small>{item.detail}</small></div></div>)}</div></>;
}
