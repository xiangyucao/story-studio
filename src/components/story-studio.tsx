    ...
    try {
      for (let index = 0; index < chapters.length; index += 1) {
        ...
        try {
          ...
          const instruction = resolveWritingLanguage(workspace.project, [chapter.title, chapterOutline?.summary || "", chapter.summary]).code === "en"
            ? "Expand the latest outline into a complete chapter. Do not add facts that conflict with established characters, world settings, or causality. Return only the complete prose."
            : "严格根据本章最新大纲扩写为完整章节，不增加与人物、世界观或事件链冲突的新事实。只输出完整正文。";
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="secondary-button" onClick={() => { setSelectedExportVolumes(new Set(exportVolumeGroups.map(exportGroupKey))); setExportOpen(true); }}><Upload size={17} />导出当前作品</button>
      ...
      {exportOpen && <div className="modal-backdrop" onMouseDown={() => setExportOpen(false)}>
        <section className="settings-modal export-modal" onMouseDown={(event) => event.stopPropagation()}>
          <div className="modal-head"><div><span className="eyebrow">EXPORT</span><h2>导出《{workspace.project.title}》</h2></div><button className="icon-button" onClick={() => setExportOpen(false)}><X size={19} /></button></div>
          <div className="export-language"><span>输出文字</span><div className="segmented"><button className={exportScript === "simplified" ? "active" : ""} onClick={() => setExportScript("simplified")}>简体中文</button><button className={exportScript === "traditional" ? "active" : ""} onClick={() => setExportScript("traditional")}>繁體中文</button></div><small>只转换导出文件，数据库中的原文不会改变。</small></div>
          <div className="export-volume-picker">
            <div className="export-volume-head"><span>选择需要导出的卷</span><button className="text-button" onClick={() => setSelectedExportVolumes(selectedExportVolumes.size === exportVolumeGroups.length ? new Set() : new Set(exportVolumeGroups.map(exportGroupKey)))}>{selectedExportVolumes.size === exportVolumeGroups.length ? "取消全选" : "全选"}</button></div>
            ...
            <small>默认选择全部；目录和正文只包含已选中的卷。</small>
          </div>
          <label className="export-toc-option"><input type="checkbox" checked={exportIncludeToc} onChange={(event) => setExportIncludeToc(event.target.checked)} /><span><strong>包含目录</strong><small>在正文前加入所选卷和章节的目录。</small></span></label>
          ...
          <div className="security-note">已选择 {selectedExportVolumes.size} / {exportVolumeGroups.length} 卷。导出文件使用作品名；繁体转换基于 OpenCC，只影响本次导出。</div>
        </section>
      </div>}
