...
export async function generateOutline(settings: ModelSettings, context: string, instruction: string, volumeCount = 7, language = "zh-CN") {
  const client = clientFor(settings);
  const system = language === "en" ? `You are a long-form fiction outline editor. Respect all established hard settings and causal chains. Follow any supplied genre, premise, and writing rules; infer missing foundations from the style reference without copying its characters, plot, proper nouns, or wording. Plan volume-level structure only, not chapters, scenes, or prose. Return exactly ${volumeCount} volume nodes, each with a clear English title and summary spanning the story's progression, turns, and ending.` : `你是长篇小说总纲编辑。严格尊重已有硬设定和事件因果。作品资料中的类型、核心构想和写作规则如已填写，必须优先遵守；如有字段为空或未设定，则从参考范本的题材倾向、核心驱动力和文体特征中推断后直接用于新故事。参考范本只用于启发结构与风格，不得复制其中的人物、情节、专有名词或原句。只规划全书的卷级结构，不规划章和场景，不写正文。必须恰好输出 ${volumeCount} 个 volume 节点，每卷都要有明确标题和介绍，依次覆盖故事的推进、转折与结局。`;
  const input = language === "en" ? `${context}\n\nUser request: ${instruction}\n\nFinal constraint: return exactly ${volumeCount} volumes and no chapters or scenes.` : `${context}\n\n用户要求：${instruction}\n\n再次确认：只输出 ${volumeCount} 卷，不要输出章或场景。`;
  if (settings.provider === "openai") {
  ...
  const raw = await generateText(settings, language === "en" ? `${system} Return JSON only: {"rationale":"...","nodes":[{"type":"volume","title":"Volume 1: ...","summary":"English volume summary..."}]}` : `${system} 必须只返回 JSON，格式为 {"rationale":"...","nodes":[{"type":"volume","title":"第一卷：...","summary":"本卷介绍..."}]}`, input);
...
export async function generateVolumeExpansion(settings: ModelSettings, context: string, currentVolume: string, instruction: string, chapterCount = 7, language = "zh-CN") {
  const client = clientFor(settings);
  const system = language === "en" ? `You are a long-form fiction structure editor. Expand the specified volume into exactly ${chapterCount} writable chapters, with optional scenes following their chapter. Every chapter needs a clear English title and an actionable English summary. Respect all characters, relationships, hard settings, and causality. Plan this volume only; do not alter or repeat other volumes and do not write prose. Never confuse a volume with a chapter.` : `你是长篇小说分卷策划编辑。把用户指定的一卷细化为恰好 ${chapterCount} 个可写作的章，可在每章后附场景。每一章必须有明确标题，并用 summary 提供可直接指导写作的章节介绍。严格尊重全书人物、关系、硬设定和事件因果。只规划这一卷，不修改或重复其他卷，不写正文。章标题必须使用目标语言惯用的章节编号或具体情节名称，绝不能把卷当成章。每个场景必须紧跟在所属章之后。`;
  const input = language === "en" ? `${context}\n\nVOLUME TO EXPAND:\n${currentVolume}\n\nUser request: ${instruction}\n\nFinal constraint: generate exactly ${chapterCount} chapters and do not treat other volumes as chapters.` : `${context}\n\n待展开的卷：\n${currentVolume}\n\n用户要求：${instruction}\n\n再次确认：恰好生成 ${chapterCount} 章，不得把其他卷当成章。`;
  if (settings.provider === "openai") {
    const response = await client.responses.parse({
      model: settings.model || process.env.OPENAI_MODEL || "gpt-5.4-mini",
      instructions: system,
  ...
  const raw = await generateText(settings, language === "en" ? `${system} Return JSON only: {"rationale":"...","nodes":[{"type":"chapter","title":"Chapter 1: ...","summary":"..."},{"type":"scene","title":"...","summary":"..."}]}` : `${system} 必须只返回 JSON，格式为 {"rationale":"...","nodes":[{"type":"chapter","title":"...","summary":"..."},{"type":"scene","title":"...","summary":"..."}]}`, input);
  const match = raw.match(/\{[\s\S]*\}/);
...
export async function generateOutlineNode(settings: ModelSettings, context: string, currentNode: string, instruction: string, language = "zh-CN") {
  const client = clientFor(settings);
  const system = language === "en" ? "You are a long-form fiction structure editor. Modify only the specified outline node, respecting all hard settings, character motivations, and causal chains. Do not write prose and do not modify any other node. Return all creative fields in English." : "你是长篇小说结构编辑。只修改用户指定的一个大纲节点，尊重全书已有硬设定、人物动机和因果链。不要扩写正文，也不要修改其他节点。";
  const input = language === "en" ? `${context}\n\nCURRENT NODE:\n${currentNode}\n\nUSER REQUEST:\n${instruction}` : `${context}\n\n当前节点：\n${currentNode}\n\n用户修改要求：${instruction}`;
  if (settings.provider === "openai") {
    const response = await client.responses.parse({
      model: settings.model || process.env.OPENAI_MODEL || "gpt-5.4-mini",
  ...
  const raw = await generateText(settings, language === "en" ? `${system} Return JSON only: {"rationale":"...","title":"...","summary":"..."}` : `${system} 必须只返回 JSON，格式为 {"rationale":"...","title":"...","summary":"..."}`, input);
  const match = raw.match(/\{[\s\S]*\}/);
