...
- 多语言写作与界面：简/繁中文、英语、德语、西班牙语、法语、日语、葡萄牙语、意大利语和韩语
- 人物关系、世界观、时间线和大纲 JSON 导入导出
- Word、Markdown 原稿、逻辑审计包、PDF/打印稿导出
...
“写作语言”可以手动指定，也可以自动识别。指定后，大纲生成、卷章展开、正文扩写、续写、改写、检查以及发送给外部模型的完整提示词都会使用对应语言。界面默认跟随作品写作语言；也可以在“模型设置 → 界面语言”中单独固定，设置只保存在当前浏览器。
...
- Word 文档（`.docx`）：包含封面、卷、章节正文和插画，可继续在 Word 中排版修改。
- PDF / 打印稿：A4 排版，包含封面、卷、已有正文和插画。在系统打印窗口选择“另存为 PDF”。
- Markdown 原稿：适合备份、Git 版本管理和迁移到其他写作工具。
- 完整作品备份（`.json`）：包含作品基石、大纲、全部正文、人物与关系、世界观、时间线、版本记录以及 Base64 编码的插画文件。

同一窗口中的“导入完整作品备份”会创建一个新的独立作品，不覆盖当前作品。系统会为项目和所有关联数据重新生成 ID，再重建大纲父子关系、章节、人物关系、事件和插画引用。因此可以用它在不同电脑之间迁移作品，也可以创建一份包含正文和历史记录的完整副本。

导出时可以选择简体或繁体，并决定是否包含目录；该选择只影响导出的文件，不会修改工作台中的原文。Word 目录包含卷、章及页码，PDF 和 Markdown 也会生成相应目录。导出文件默认使用作品名，空白计划章节不会进入成稿。
